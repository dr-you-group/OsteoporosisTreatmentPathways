...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (40174359)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (40174359)
  and c.invalid_reason is null
) I
) C UNION ALL 
SELECT 2 as codeset_id, c.concept_id FROM (select distinct I.concept_id FROM
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (19126449)
  UNION 
  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (19126449)
  and c.invalid_reason is null
...
from (
  select de.person_id, de.drug_exposure_id, de.drug_exposure_start_date, de.drug_exposure_end_date, de.drug_concept_id, datediff(day, de.drug_exposure_start_date, de.drug_exposure_end_date)+1 as 'days_supply', 90 as 'duration'
...
union all
  select de.person_id, de.drug_exposure_id, de.drug_exposure_start_date, de.drug_exposure_end_date, de.drug_concept_id, datediff(day, de.drug_exposure_start_date, de.drug_exposure_end_date)+1 as 'days_supply', 365 as 'duration'
  FROM @cdm_database_schema.DRUG_EXPOSURE de

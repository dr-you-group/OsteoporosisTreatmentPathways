...
CREATE TABLE @cohortDatabaseSchema.@cohortTable_SUBJECTS_EVENTS (
	SUBJECT_ID BIGINT,
	COMBO_ID VARCHAR(100),
	FIRST_START_DATE VARCHAR(100),
	SECOND_COMBO VARCHAR(100),
	SECOND_START_DATE VARCHAR(100),
	THIRD_COMBO VARCHAR(100),
	THIRD_START_DATE VARCHAR(100)
);
...
SELECT T1.SUBJECT_ID, cast(COMBO_ID as varchar) AS FIRST_COMBO
     , cast(COHORT_START_DATE as varchar) AS FIRST_START_DATE
...
SELECT *
FROM (
	SELECT T1.SUBJECT_ID, cast(COMBO_ID as varchar) AS FIRST_COMBO, cast(COHORT_START_DATE as varchar) AS FIRST_START_DATE,
	  cast(LEAD(COMBO_ID) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as varchar) AS SECOND_COMBO,
	  cast(LEAD(COHORT_START_DATE) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as VARCHAR) AS SECOND_START_DATE
	  , NULL AS THIRD_COMBO
	  , NULL AS THIRD_START_DATE
	FROM @cohortDatabaseSchema.@cohortTable_OSTEOPOROSIS_PATHWAY_ANALYSIS_EVENTS T1, #ODTP_SUBJECTS T2
...
SELECT T1.SUBJECT_ID, cast(COMBO_ID as varchar) AS FIRST_COMBO, cast(COHORT_START_DATE as varchar)  AS FIRST_START_DATE,
  cast(LEAD(COMBO_ID) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as varchar) AS SECOND_COMBO,
  cast(LEAD(COHORT_START_DATE) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as varchar) AS SECOND_START_DATE,
  cast(LEAD(COMBO_ID,2) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as varchar) AS THIRD_COMBO,
  cast(LEAD(COHORT_START_DATE,2) OVER (PARTITION BY T1.SUBJECT_ID ORDER BY COHORT_START_DATE) as varchar) AS THIRD_START_DATE
FROM @cohortDatabaseSchema.@cohortTable_OSTEOPOROSIS_PATHWAY_ANALYSIS_EVENTS T1, #ODTP_SUBJECTS T2
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS COHORT_ID
			, SUBJECTS.SUBJECT_ID
			, cast(SUBJECTS.FIRST_START_DATE as DATE) AS LINE_START_DATE
		...
		ON SUBJECTS.SUBJECT_ID=COHORTS.SUBJECT_ID AND cast(SUBJECTS.FIRST_START_DATE as DATE)=COHORTS.COHORT_START_DATE
	...
	(SELECT *, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			...
			, SUBJECTS.SUBJECT_ID
			, cast(SUBJECTS.FIRST_START_DATE as DATE) AS LINE_START_DATE
			, cast(SUBJECTS.SECOND_START_DATE as DATE) AS LINE_END_DATE -- SECOND LINE
			, OP.OBSERVATION_PERIOD_END_DATE AS LAST_VISIT_DATE
		FROM @cohortDatabaseSchema.@cohortTable_SUBJECTS_EVENTS SUBJECTS
		JOIN @cohortDatabaseSchema.@cohortTable COHORTS
		ON SUBJECTS.SUBJECT_ID=COHORTS.SUBJECT_ID AND cast(SUBJECTS.FIRST_START_DATE as DATE)=COHORTS.COHORT_START_DATE
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) AND SUBJECTS.SECOND_COMBO IS NOT NULL AND cast(SUBJECTS.SECOND_COMBO as int) IN (1,2,4,8,16,32,64)
	) T1,
	(SELECT *, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
...
FROM (
		...
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) AND SUBJECTS.SECOND_COMBO IS NOT NULL AND cast(SUBJECTS.SECOND_COMBO as int) NOT IN (1,2,4,8,16,32,64)
	) T1,
	(SELECT *, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
WHERE T1.SUBJECT_ID = T2.SUBJECT_ID AND T2.COHORT_START_DATE >= T1.LINE_START_DATE AND T2.COHORT_START_DATE <= T1.LINE_END_DATE  AND T1.COHORT_ID=T2.DRUG_ID;
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS COHORT_ID
			, SUBJECTS.SUBJECT_ID
			, cast(SUBJECTS.SECOND_START_DATE as DATE) AS LINE_START_DATE
			, COHORTS.COHORT_END_DATE AS LINE_END_DATE
			, OP.OBSERVATION_PERIOD_END_DATE AS LAST_VISIT_DATE
		FROM @cohortDatabaseSchema.@cohortTable_SUBJECTS_EVENTS SUBJECTS
		JOIN @cohortDatabaseSchema.@cohortTable COHORTS
		ON SUBJECTS.SUBJECT_ID=COHORTS.SUBJECT_ID AND cast(SUBJECTS.SECOND_START_DATE as DATE)=COHORTS.COHORT_START_DATE
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			...
			, SUBJECTS.SUBJECT_ID
			, cast(SUBJECTS.SECOND_START_DATE as DATE) AS LINE_START_DATE
			, cast(SUBJECTS.THIRD_START_DATE as DATE) AS LINE_END_DATE
			, OP.OBSERVATION_PERIOD_END_DATE AS LAST_VISIT_DATE
		FROM @cohortDatabaseSchema.@cohortTable_SUBJECTS_EVENTS SUBJECTS
		JOIN @cohortDatabaseSchema.@cohortTable COHORTS
		ON SUBJECTS.SUBJECT_ID=COHORTS.SUBJECT_ID AND cast(SUBJECTS.SECOND_START_DATE as DATE)=COHORTS.COHORT_START_DATE
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) AND cast(SUBJECTS.SECOND_COMBO as int) IN (1,2,4,8,16,32) AND SUBJECTS.THIRD_COMBO IS NOT NULL
	) T1,
	(SELECT *, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
...
FROM (
		...
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) AND cast(SUBJECTS.SECOND_COMBO as int) NOT IN (1,2,4,8,16,32) AND SUBJECTS.THIRD_COMBO IS NOT NULL
	) T1,
	(SELECT *, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS DRUG_ID FROM @cohortDatabaseSchema.@cohortTable_DRUG) T2
WHERE T1.SUBJECT_ID = T2.SUBJECT_ID AND T2.COHORT_START_DATE >= T1.LINE_START_DATE AND T2.COHORT_START_DATE <= T1.LINE_END_DATE;
...
FROM (
		SELECT COHORTS.COHORT_DEFINITION_ID
			, RIGHT(cast(COHORT_DEFINITION_ID as varchar(5)), 1) AS COHORT_ID
			, SUBJECTS.SUBJECT_ID
			, cast(SUBJECTS.THIRD_START_DATE as DATE) AS LINE_START_DATE
			, COHORTS.COHORT_END_DATE AS LINE_END_DATE
			, OP.OBSERVATION_PERIOD_END_DATE AS LAST_VISIT_DATE
		FROM @cohortDatabaseSchema.@cohortTable_SUBJECTS_EVENTS SUBJECTS
		JOIN @cohortDatabaseSchema.@cohortTable COHORTS
		ON SUBJECTS.SUBJECT_ID=COHORTS.SUBJECT_ID AND cast(SUBJECTS.THIRD_START_DATE as DATE)=COHORTS.COHORT_START_DATE
		JOIN @cdmDatabaseSchema.OBSERVATION_PERIOD OP
		ON SUBJECTS.SUBJECT_ID=OP.PERSON_ID
		WHERE COHORTS.COHORT_DEFINITION_ID IN (1001,1002,1003,1004,1005,1006) AND cast(SUBJECTS.SECOND_COMBO as int) NOT IN (1,2,4,8,16,32) AND SUBJECTS.THIRD_COMBO IS NOT NULL
	) T1,
